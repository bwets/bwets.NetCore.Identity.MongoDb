...
namespace bwets.NetCore.Identity.MongoDb.Stores
{
	...
	{
		private readonly Collection<Guid, TRole> _roleCollection;

		private readonly Collection<Guid, TUser> _userCollection;

		public UserStore(Collection<Guid, TUser> userCollection, Collection<Guid, TRole> roleCollection)
		{
		...
		{
			return (await _userCollection.FirstOrDefaultAsync(x => x.Id == user.Id))?.Roles
			       ?.Select(roleId => _roleCollection.FirstOrDefault(x => x.NormalizedName == roleId))
			       .Where(x => x != null)
			       .Select(x => x.Name)
